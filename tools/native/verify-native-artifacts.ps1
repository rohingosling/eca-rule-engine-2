...
function Assert-VersionInformation
{
    ...
    # Handle the branch where version information.file version or product version differs from 2.0.0, company name
    ...
    if (
        ( $VersionInformation.FileVersion -cne '2.0.0' ) -or
        ( $VersionInformation.ProductVersion -cne '2.0.0' ) -or
...
function Invoke-IsolatedClientSmoke
{
    ...
    try
    {
        ...
        )
        {
            try
            {
                ...
                foreach ( $automationElement in $automationElements )
                {
                    ...
                    if (
                        $automationElement.Current.Name -eq 'About' -and
                        $accessibleElementCount -ge 5
                    )
                    {
                        $windowPattern = $automationElement.GetCurrentPattern(
                            [System.Windows.Automation.WindowPattern]::Pattern
        ...
        {
            throw 'The native client did not complete its rendered smoke within forty-five seconds.'
        }
        ...
        {
            throw 'Windows UI Automation did not observe and cancel the native file chooser.'
        }

        if ( -not $javaFxDialogObserved )
        {
            throw 'Windows UI Automation did not observe and close the rendered JavaFX dialog.'
        }
