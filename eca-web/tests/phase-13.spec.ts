...
test ( "loads every shell asset beneath the GitHub Pages project path", async ( { page } ) =>
{
    ...
    await page.reload ();
    await expect ( page ).toHaveTitle ( "ECA Rule Engine Laboratory (Version 2.1.0 - Untitled)" );
    await expect ( page.getByRole ( "heading", { level: 1 } ) ).toContainText ( "Version 2.1" );
            ...
            menuElement =>
            {
                ...
                return {
                    ...
                    labels: menuItems.map (
                        menuItem =>
                        {
                            const label = menuItem.querySelector <HTMLElement> ( ".menu-label" );
...
test ( "provides standard panel defaults and bounded splitter travel", async ( { page } ) =>
{
    ...
    const consoleMaximum = await diagnosticsSplitter.evaluate (
        splitter => Math.floor ( splitter.parentElement!.clientHeight * 2 / 3 )
    );

    await expect ( diagnosticsSplitter ).toHaveAttribute ( "aria-valuemin", "122" );
    await expect ( diagnosticsSplitter ).toHaveAttribute ( "aria-valuemax", String ( consoleMaximum ) );
    await expect ( diagnosticsSplitter ).toHaveAttribute ( "aria-valuenow", "196" );
    ...
    await expect ( diagnosticsSplitter ).toHaveAttribute ( "aria-valuenow", String ( consoleMaximum ) );
    ...
    const resizedConsoleMaximum = await diagnosticsSplitter.evaluate (
        splitter => Math.floor ( splitter.parentElement!.clientHeight * 2 / 3 )
    );

    await expect ( diagnosticsSplitter ).toHaveAttribute ( "aria-valuemin", "122" );
    await expect ( diagnosticsSplitter ).toHaveAttribute ( "aria-valuemax", String ( resizedConsoleMaximum ) );
} );
...
test ( "matches the accepted light and dark visual baselines", async ( { page } ) =>
{
    ...
    await expect (
        page.getByRole ( "separator", { name: "Resize Console pane" } )
    ).toHaveAttribute (
        "aria-valuenow",
        "196"
    );
