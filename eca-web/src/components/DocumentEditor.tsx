        ...
        >
            <h2>
                { properties.formState.originalEntityIdentifier === null
                    ? text ( "editor.entity.add.heading" )
                    : text ( "editor.entity.edit.heading" ) }
                { ": " }
                { text ( CATEGORY_LABEL_KEYS [ properties.category ] ) }
            </h2>
            <IdentityFields
            ...
            <div className="form-actions">
                <button type="submit">{ text ( "button.apply" ) }</button>
        ...
        >
            ...
            <GroupBox legend={ text ( "model.overview.group" ) }>
                ...
                <div className="group-actions">
                    <button type="submit">{ text ( "button.apply.model.details" ) }</button>
                    <button onClick={ properties.onValidate } type="button">{ text ( "button.validate" ) }</button>
                </div>
            </GroupBox>
            <GroupBox legend={ text ( "editor.summary.group" ) }>
                <div className="document-summary-grid">
                    { ( Object.keys ( CATEGORY_LABEL_KEYS ) as ModelCategory[] ).map (
                        category => (
                            <div key={ category }>
                                <span>{ text ( CATEGORY_LABEL_KEYS [ category ] ) }</span>
                                <strong>{ properties.content.entityIdentifiers.get ( category )?.length ?? 0 }</strong>
                            </div>
                        )
                    ) }
                </div>
    ...
    return (
        <section className="editor-content">
            ...
            <GroupBox legend={ categoryLabel }>
                ...
                <div className="group-actions">
                    <button onClick={ addEntity } type="button">{ text ( "button.add" ) }</button>
                    <button disabled={ selectedIdentifier === null } onClick={ duplicateSelectedEntity } type="button">
                        { text ( "button.duplicate" ) }
                    </button>
                    <button disabled={ selectedIdentifier === null } onClick={ editEntity } type="button">
                        { text ( "button.edit" ) }
                    </button>
                    <button
                        aria-describedby={ selectedIdentifier === null ? undefined : "entity-reference-preview" }
                        disabled={ selectedIdentifier === null || references.length > 0 }
