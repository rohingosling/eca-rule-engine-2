...
Domain independent, stateless **ECA Rule Engine** reference implementation, Version 2.1.0.<br>Companion implementation for the [Stateless ECA Rule Engine](https://zenodo.org/records/21804777) technical note.


<p align="center">
  <img alt="Animated walkthrough of ECA Rule Engine Laboratory" src="assets/gif/demo-4.gif">
</p>
...
This project implements a domain-independent, stateless, ECA (**E**vent **C**ondition **A**ction) rule engine reference solution that accompanies the [Stateless ECA Rule Engine](https://zenodo.org/records/21804777) technical note.
...
1. open [**ECA Rule Engine Laboratory**](https://rohingosling.github.io/eca-rule-engine-2/) in your browser, and wait for the built-in server's Ready entry in the structured **Console**.
...
2. Download both release files into a folder: [`eca-server.exe`](https://github.com/rohingosling/eca-rule-engine-2/releases/download/v2.1.0/eca-server.exe) and [`eca-client.exe`](https://github.com/rohingosling/eca-rule-engine-2/releases/download/v2.1.0/eca-client.exe).
...
- A context-aware `User Guide` panel that surfaces sections of the [Stateless ECA Rule Engine](https://zenodo.org/records/21804777) technical note relevant to the component being edited or studied in the editor.
- A structured, filterable `Console` with severity, source, code, message, and context columns.
...
1. Start the server in **PowerShell**.
   ...
   Started without arguments, `run-server.bat` hosts the bundled example model, `examples\eca-rule-engine-example.json`. To host a different model, pass the command and its arguments through explicitly.

   ```powershell
   run-server.bat start --model <model-name.json>
   ```
...
3. There are two ways to load a model into **ECA Rule Engine Laboratory**. You can load the model from **disk**, or you can pull the model from the running **server**.
   ...
   - **Pull the model being hosted by the server**
     ...
     ***Note:**<br>This will only work if the server is hosting a model, which it is by default, and which it also is whenever it was started with an explicit `--model <file name>` argument.*
