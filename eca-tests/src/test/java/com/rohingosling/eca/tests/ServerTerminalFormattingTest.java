...
final class ServerTerminalFormattingTest
{
    ...
    //-----------------------------------------------------------------------------------------------------------------
    // Method: terminalFilter_withholdsFrameworkWarningsAndErrors
    //
    // Description:
    //
    //   Verifies that the terminal withholds warnings and errors raised outside the rule engine.
    //
    //-----------------------------------------------------------------------------------------------------------------

    @Test
    void terminalFilter_withholdsFrameworkWarningsAndErrors ()
    {
        // Verify that a warning or an error raised by a framework, a library, or the Java runtime is withheld.

        assertThat (
            ServerTerminalOutput.isReportableRecord (
                logRecord ( Level.WARNING, "io.helidon.webserver.LoomServer", "startIt", "Framework warning." )
            )
        ).isFalse ();
        assertThat (
            ServerTerminalOutput.isReportableRecord (
                logRecord ( Level.SEVERE, "com.fasterxml.jackson.databind.ObjectMapper", "readValue", "Library error." )
            )
        ).isFalse ();
        assertThat (
            ServerTerminalOutput.isReportableRecord (
                logRecord ( Level.WARNING, "sun.misc.Unsafe", "objectFieldOffset", "Deprecated runtime method." )
            )
        ).isFalse ();
    }

    //-----------------------------------------------------------------------------------------------------------------
    // Method: terminalFilter_reportsProductWarningsErrorsAndFrameworkInformation
    //
    // Description:
    //
    //   Verifies that the terminal reports rule-engine warnings and errors alongside framework information.
    //
    //-----------------------------------------------------------------------------------------------------------------

    @Test
    void terminalFilter_reportsProductWarningsErrorsAndFrameworkInformation ()
    {
        // Verify that a warning or an error raised by the rule engine reaches the terminal.
        ...
        assertThat (
            ServerTerminalOutput.isReportableRecord (
                logRecord (
                    ...
                    "com.rohingosling.eca.server.StructuredHostedModelLogger",
                    "log",
                    "event=model-replacement status=rejected"
                )
            )
        ).isTrue ();
        assertThat (
            ServerTerminalOutput.isReportableRecord (
                logRecord ( Level.SEVERE, "com.rohingosling.eca.server.EcaHttpServer", "start", "Product error." )
            )
        ).isTrue ();

        // Verify that the restriction covers warnings and errors only, so framework information is unaffected.

        assertThat (
            ServerTerminalOutput.isReportableRecord (
                logRecord ( Level.INFO, "io.helidon.webserver.LoomServer", "startIt", "Started all channels." )
            )
        ).isTrue ();
    }
    ...
    {
        // Return the result produced by log record.

        return logRecord ( Level.INFO, sourceClassName, sourceMethodName, message );
    }
